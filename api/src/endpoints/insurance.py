...
from schemas.schemas import InsuranceBaseCreate, InsuranceCalculation, InsuranceRate
...
async def calculate_insurance_(insurance:InsuranceRate):
    base_tariff = await get_insurance_base(date= date.today(), cargo_type=insurance.cargo_type)
    if not base_tariff is None:
        cost = base_tariff.base_value * insurance.rate
